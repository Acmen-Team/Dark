...
{
	VectorRegister Vec2;
	Vec2.V[0] = Math::Abs(Vec.V[0]);
	Vec2.V[1] = Math::Abs(Vec.V[1]);
	Vec2.V[2] = Math::Abs(Vec.V[2]);
	Vec2.V[3] = Math::Abs(Vec.V[3]);
	return Vec2;
}
...
{
	return MakeVectorRegister(1.0f / Math::Sqrt(Vec.V[0]), 1.0f / Math::Sqrt(Vec.V[1]), 1.0f / Math::Sqrt(Vec.V[2]), 1.0f / Math::Sqrt(Vec.V[3]));
}
...
FORCEINLINE VectorRegister VectorReciprocalLen(const VectorRegister& Vector)
{
	...
	float rlen = 1.0f / Math::Sqrt(Len.V[0]);
...
FORCEINLINE void VectorSinCos(VectorRegister* VSinAngles, VectorRegister* VCosAngles, const VectorRegister* VAngles)
{
	...
	Math::SinCos(&VecSin.f[0], &VecCos.f[0], VecAngles.f[0]);
	Math::SinCos(&VecSin.f[1], &VecCos.f[1], VecAngles.f[1]);
	Math::SinCos(&VecSin.f[2], &VecCos.f[2], VecAngles.f[2]);
	Math::SinCos(&VecSin.f[3], &VecCos.f[3], VecAngles.f[3]);
...
{
	return !Math::IsFinite(Vec.V[0]) || !Math::IsFinite(Vec.V[1]) || !Math::IsFinite(Vec.V[2]) || !Math::IsFinite(Vec.V[3]);
}
...
{
	return MakeVectorRegister(
		Math::Atan2(VectorGetComponent(X, 0), VectorGetComponent(Y, 0)),
...
{
	return MakeVectorRegister(
		Math::Fmod(VectorGetComponent(X, 0), VectorGetComponent(Y, 0)),
