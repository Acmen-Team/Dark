...
LinearColor::LinearColor(const Vector4& Vector) :
	R(Vector.X),
	G(Vector.Y),
	B(Vector.Z),
	A(Vector.W)
...
Color LinearColor::ToRGBE() const
{
	...
	else
	{
		...
		newColor.R = (uint8)Math::Clamp(Math::TruncToInt(R * Scale), 0, 255);
		newColor.G = (uint8)Math::Clamp(Math::TruncToInt(G * Scale), 0, 255);
		newColor.B = (uint8)Math::Clamp(Math::TruncToInt(B * Scale), 0, 255);
		newColor.A = (uint8)(Math::Clamp(Exponent, -128, 127) + 128);
	}
 ...
 */
LinearColor LinearColor::MakeFromHSV8(uint8 H, uint8 S, uint8 V)
{
	// want a given H value of 255 to map to just below 360 degrees
	const LinearColor HSVColor((float)H * (360.0f / 256.0f), (float)S / 255.0f, (float)V / 255.0f);
...
LinearColor LinearColor::MakeRandomColor()
{
	...
	return LinearColor::MakeFromHSV8(Hue, 255, 255);
}
...
float LinearColor::EvaluateBezier(const LinearColor* ControlPoints, int32 NumPoints, TArray<LinearColor>& OutPoints)
{
	//check(ControlPoints);
	//check(NumPoints >= 2);
...
Color LinearColor::ToFColor(const bool bSRGB) const
{
	...
	Result.A = (uint8)Math::FloorToInt(FloatA * 255.999f);
	Result.R = (uint8)Math::FloorToInt(FloatR * 255.999f);
	Result.G = (uint8)Math::FloorToInt(FloatG * 255.999f);
	Result.B = (uint8)Math::FloorToInt(FloatB * 255.999f);
...
{
	return Color(
		(uint8)Math::Clamp<int32>(Math::TruncToInt(R * 255.f), 0, 255),
		(uint8)Math::Clamp<int32>(Math::TruncToInt(G * 255.f), 0, 255),
		(uint8)Math::Clamp<int32>(Math::TruncToInt(B * 255.f), 0, 255),
		(uint8)Math::Clamp<int32>(Math::TruncToInt(A * 255.f), 0, 255)
	);
}
...
Color Color::MakeRedToGreenColorFromScalar(float Scalar)
{
	...
	const uint8 R = (uint8)Math::TruncToInt(255 * RedSclr);
	const uint8 G = (uint8)Math::TruncToInt(255 * GreenSclr);
	const uint8 B = 0;
	return Color(R, G, B);
}
